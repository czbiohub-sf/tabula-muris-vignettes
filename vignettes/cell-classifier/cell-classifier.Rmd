...
We start by installing the packages from bioconductor:
```{r}
...
As we briefly mentioned in the previous post, instead of manually assigning which cells we should be comparing and verifying if there is any overlap, we want instead to build a map where we can project new experiments.
We want to use Tabula muris as the reference dataset. because the data was annotated by field experts we want to trust such labels as ground-true and build on the top of that -- this might not be necessarilly the case and we should design a strategy to select the "properly" annotated cells, still need to explore this in much more detail... For now ```Seurat```&```scmap```...
...
We start our quest for automatic ways of classifying data using Seurat

The libraries needed are
```{r}
# load(file=here("00_data_ingest", "04_tissue_robj_generated", "facs_Bladder_seurat_tiss.Robj"))
tissFACS <- tm.facs.tiss
# load(file=here("00_data_ingest", "04_tissue_robj_generated", "droplet_Bladder_seurat_tiss.Robj"))
tissDROPLET <- tm.droplet.tiss
```

Build the random forest classifier to be used with ClassifyCells
Useful if you want to use the same classifier with several sets of new data

```{r}
FACS_classes <- tissFACS@meta.data$cell_ontology_class
classifierFACS <- BuildRFClassifier(tissFACS, training.classes = FACS_classes)
...
classifierDROPLET <- BuildRFClassifier(tissDROPLET, training.classes = tissDROPLET@meta.data$cell_ontology_class)
...
tissFACS@meta.data$cell_ontology_class <- replace(tissFACS@meta.data$cell_ontology_class, is.na(tissFACS@meta.data$cell_ontology_class), "notKnown") 
FACS_classes = as_factor(tissFACS@meta.data$cell_ontology_class)
classifierFACS <- BuildRFClassifier(tissFACS, training.classes = tissFACS@meta.data$cell_ontology_class)
```
...
The union trick is necessary to account for the fact that sometimes the number of levels in the data and in the classifier don't match


Because this classifycells *always* classifies a cell we want to change to something that give us greys...
