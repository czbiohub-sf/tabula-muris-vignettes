...
Run Principal Component Analysis.
```{r, fig.height=4, fig.width=8}
tiss_facs_bcells <- RunICA(object = tiss_facs_bcells)
tiss_facs_bcells <- ProjectDim(object = tiss_facs_bcells, reduction.type = 'ica')
ICHeatmap(object = tiss_facs_bcells, pc.use = 1:3, cells.use = 500, do.balanced = TRUE, label.columns = FALSE, num.genes = 8)
```
...
```{r}
tiss_facs_cd19 = SubsetData(object = tiss_facs_bcells, ident.use = c(0, 1, 3))
...
```{r}
cluster.markers %>% group_by(cluster) %>% top_n(-6, p_val_adj)
```
...
```{r}
annotation = 'tissue'
annotations = tiss_facs_cd19@meta.data[,annotation]
unique_annotations = unique(annotations)
enumerated_annotations = 0:(length(unique_annotations)-1)
```

```{r}
organ_markers[, 'tissue'] = as.factor(plyr::mapvalues(x=organ_markers$cluster, from=enumerated_annotations, to=unique_annotations))
head(organ_markers)
```
...
```{r}
FeaturePlot(tiss_facs_cd19, features.plot=c('Cd74'))
```
...
tiss_facs_cd19 <- FindClusters(object = tiss_facs_cd19, reduction.type = "ica", dims.use = 1:sub.n.pcs,
    resolution = sub.res.used, print.output = 0, save.SNN = TRUE)
```
