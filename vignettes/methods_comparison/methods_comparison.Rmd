...
This question is fundamental because we want to be able to integrate datasets produced by different labs that report on similar biological specimens. In creating [Tabula muris](https://github.com/czbiohub/tabula-muris-vignettes/blob/master/README.md) we used two different technical approaches:
<ol>
  <li><span style="font-weight: bold;">microfluidic droplet-based 3’-end counting</span> enabled the survey of thousands of cells at relatively low coverage;</li>
  <li><span style="font-weight: bold;">FACS-based full length transcript analysis</span> enabled characterization of cell types with high sensitivity and coverage.</li>
</ol>
...
You can view the graphical output [here](https://github.com/czbiohub/tabula-muris-vignettes/blob/master/vignettes/methods_comparison/methods_comparison_diffExp.pdf)
...
This simple method illustrates one way of comparing datasets that have been independently obtained without the need to re-analize the raw data. We have, however, assumed that the annotations and the analysis are consistent across the datasets. For this case in particular, the assumption is valid (within certain limits) because all three datasets compared where analyzed using [Seurat](http://satijalab.org/seurat/) R package and the overlap among cell types was inferred using the gene symbol.

Next we will be looking at using machine learning techniques to derive a better, scalable comparisons pipeline.
