...
For the microwellseq data we're using the `.rds` made available by the Satija lab alongside with the annotations made available to us by the authors. Follow the steps [here](https://github.com/czi-hca-comp-tools/easy-data/blob/MCA/datasets/mouse-cell-atlas/MCA.md) to download the data and then:
...
tiss.name.tm.facs
print("Tissues available for microwell-seq")
tiss.name.microwellseq
```

Let us use ```Bladder`` as the organ of interest:
```{r}
droplet.tiss.interest = "Marrow"
facs.tiss.interest = "Marrow"
microwell.tiss.interest = "Bone-Marrow"
```
...
microwell.metadata.tiss <- microwell.metadata[microwell.metadata$Tissue==microwell.tiss.interest,]
microwell.matrix.tiss <- microwell.matrix[,microwell.metadata.tiss$Cell.name]

microwell.metadata.tiss.ann <- t(as.data.frame(strsplit(microwell.metadata.tiss$Cell.Anno,"[()]")))
row.names(microwell.metadata.tiss.ann) <- microwell.metadata.tiss$Cell.name
colnames(microwell.metadata.tiss.ann) <- c("cell_type1","tissue")
...
```{r microwell}
sce.microwell <- selectFeatures(sce.microwell, suppress_plot = FALSE, n_features = 250)
...
sce.microwell <- indexCluster(sce.microwell)
head(metadata(sce.microwell)$scmap_cluster_index)
heatmap(as.matrix(metadata(sce.microwell)$scmap_cluster_index),margins=c(12,1),cexCol=0.63, labRow = "")
```
...
#### microwell-seq data self projection
...
#### microwell-seq projected into Tabula muris FACS
...
#### microwell-seq projected into Tabula muris 10x
...
#### Tabula muris 10x projected into microwell-seq
...
#### Tabula muris FACS projected into microwell-seq
...
sce.microwell_cell <- indexCell(sce.microwell)
```
...
names(metadata(sce.microwell_cell)$scmap_cell_index)
```
...
length(metadata(sce.microwell_cell)$scmap_cell_index$subcentroids)
dim(metadata(sce.microwell_cell)$scmap_cell_index$subcentroids[[1]])
metadata(sce.microwell_cell)$scmap_cell_index$subcentroids[[1]][,1:5]
```
...
dim(metadata(sce.microwell_cell)$scmap_cell_index$subclusters)
metadata(sce.microwell_cell)$scmap_cell_index$subclusters[1:5,1:5]
```
...
scmapCell_results.microwell <- scmapCell(
  sce.microwell_cell, 
  list(
    microwell = metadata(sce.microwell_cell)$scmap_cell_index
  ),
  w = 10
)
```
...
#### Tabula muris 10x data projected into microwell-seq using scmap-cell
```{r}
plot(
  getSankey(
    colData(sce.tm_droplet_cell)$cell_type1, 
...
#### Tabula muris FACS data projected into microwell-seq using scmap-cell
```{r}
plot(
  getSankey(
    colData(sce.tm_facs_cell)$cell_type1,  
...
#### microwell-seq data self-projection using scmap-cell
```{r}
plot(
  getSankey(
    colData(sce.microwell_cell)$cell_type1, 
    scmapCell_clusters.microwell$scmap_cluster_labs[,"microwell"],
    plot_height = 400,
    colors = c('#009ACD', '#9ACD32', '#FF8247','#D02090', '#636363','#000080','#CD853F','#8B0000','#FF6347',
               '#000000','#CD1076','#BFEFFF','#698B22','#CD6889','#FFFF00','#CDBA96','#D02090','#CD4F39',
...
#### microwell-seq data projected into Tabula muris 10x using scmap-cell
```{r}
plot(
  getSankey(
    colData(sce.microwell_cell)$cell_type1,  
...
#### microwell-seq data projected into Tabula muris FACS using scmap-cell
```{r}
plot(
  getSankey(
    colData(sce.microwell)$cell_type1, 
