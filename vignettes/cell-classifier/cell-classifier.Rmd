...
As we briefly mentioned in the previous post, instead of manually and arbitrarily deciding which cell ontologies we should be comparing and computing the overlaps at the level of differentially expressed genes, we want to have a method to map on the fly new experimental data to existing reference datasets.

Given the broad collection of organs in Tabula muris it is natural to think of it as a possible reference dataset. Here we will exemplify how we can build the reference classifier using the buil-in functions of ```Seurat``` and the recently published ```scmap``` method.

We can access Tabula muris data [here](https://github.com/czbiohub/tabula-muris-vignettes/tree/experiment-comparisons/data):
```{r}
...
## Seurat build-in functions for cell classification
